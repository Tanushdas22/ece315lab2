...
int main(void)
{
    ...
    q_uart_to_rgb       = xQueueCreate(1, sizeof(uart_cmd_t));  /* length 1 for xQueueOverwrite */
    q_uart_to_ssd       = xQueueCreate(1, sizeof(uart_cmd_t));

    xTaskCreate(vUART_RX_Task,   "UART_RX",   512, NULL, 3, NULL);
    xTaskCreate(vUART_TX_Task,   "UART_TX",   512, NULL, 3, NULL);
    xTaskCreate(vUART_ParseTask, "UART_Parse", 1024, NULL, 3, NULL);
    xTaskCreate(vKeypadTask,     "Keypad",    configMINIMAL_STACK_SIZE, NULL, 3, NULL);
    xTaskCreate(vButtonsTask,    "Buttons",   configMINIMAL_STACK_SIZE, NULL, 3, NULL);
    xTaskCreate(vRGBTask,        "RGB",       configMINIMAL_STACK_SIZE, NULL, 3, NULL);
    xTaskCreate(vDisplayTask,    "Display",   configMINIMAL_STACK_SIZE, NULL, 3, NULL);
    
    /* Do NOT print before scheduler - no task drains q_tx yet */
    vTaskStartScheduler();
    while (1);
    return 0;
}
...
static void vUART_TX_Task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (has_pending && uart_tx_byte_nonblocking((uint8_t)pending))
            has_pending = 0;
        /* Short delay when we have pending TX, else yield briefly */
        vTaskDelay(pdMS_TO_TICKS(has_pending ? 1 : POLL_DELAY_MS));
    }
}
...
static void vUART_ParseTask(void *pvParameters)
{
    ...
    /* Startup message after scheduler - UART_TX_Task is now running */
    ...
    for (;;) {
        ...
        print_string("\r\n*******************************************\r\n");
        print_string("Menu:\r\n");
        print_string("1. Set RGB brightness (0-20)\r\n");
        print_string("2. Set RGB color (0-6: R,G,B,M,C,Y,W)\r\n");
        print_string("3. Set SSD display (two digits)\r\n");
        print_string("\r\nEnter your option: ");

        /* Use receive_string - reads until \r, no flush_uart to eat valid input */
        receive_string(buf, sizeof(buf));
        op = (strlen(buf) > 0) ? buf[0] : 0;

        print_string("\r\n*******************************************\r\n");

        switch (op) {
            case '1':
                print_string("Enter brightness (0-20): ");
                receive_string(buf, sizeof(buf));
                if (strlen(buf) > 0) {
                    int a;
                    if (sscanf(buf, "%d", &a) == 1) {
                        uart_cmd_t cmd = { CMD_RGB_BRIGHT, (u32)(a < 0 ? 0 : (a > 20 ? 20 : a)), 0 };
                        xQueueOverwrite(q_uart_to_rgb, &cmd);
                        print_string("Brightness set.\r\n");
                    }
                }
                break;
            case '2':
                print_string("Enter color (0=RED,1=GREEN,2=BLUE,3=MAGENTA,4=CYAN,5=YELLOW,6=WHITE): ");
                receive_string(buf, sizeof(buf));
                if (strlen(buf) > 0) {
                    int a;
                    if (sscanf(buf, "%d", &a) == 1 && a >= 0 && a < 7) {
                        uart_cmd_t cmd = { CMD_RGB_COLOR, (u32)a, 0 };
                        xQueueOverwrite(q_uart_to_rgb, &cmd);
                ...
                break;
            case '3':
                print_string("Enter two digits (e.g. 42): ");
                receive_string(buf, sizeof(buf));
                if (strlen(buf) >= 2) {
                    int a = buf[0] - '0', b = buf[1] - '0';
                    if (a >= 0 && a <= 9 && b >= 0 && b <= 9) {
                        uart_cmd_t cmd = { CMD_SSD_DISPLAY, (u32)('0' + a), (u32)('0' + b) };
                        xQueueOverwrite(q_uart_to_ssd, &cmd);
                        print_string("SSD set.\r\n");
                    }
                }
                break;
            default:
                print_string("Option not recognized.\r\n");
                break;
        }
    }
}
...
static void vRGBTask(void *pvParameters)
{
    ...
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(xOnDelay));
        XGpio_DiscreteWrite(&RGB_LEDInst, RGB_CHANNEL, 0);
        vTaskDelay(pdMS_TO_TICKS(xOffDelay));
    }
}
